...
void setup() 
{
	...
	Serial.println("This is an example of how to use the STM32F3 RTC library.");
	Serial.println("Hit enter to start");

	read_line();

	rtc.begin(RTCSEL_LSE);
...
void wfisleep(uint16_t delay) {
	for(int i=0; i<delay; i++)
	{
		systick_triggered = false;
		while(!systick_triggered) asm("wfi");
	}
	systick_triggered = false;
}
