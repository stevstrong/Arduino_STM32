...
//------------------------------------------------------------------------------
WS2812B::WS2812B(uint16_t number_of_leds, SPIClass * _spi) :
  begun(false), brightness(0), pixels(NULL), doubleBuffer(NULL), spi(_spi)
{
...
{
  if (!begun) {
    // need bit rate of 400nS but closest we can do @ 72Mhz is 444ns (which is within spec)
    // spi->setClockDivider(SPI_CLOCK_DIV32);
    spi->setFrequency(4000000);
    spi->begin();
    // spi->beginTransaction(SPISettings(1e6));
    begun = true;
  }
}
...
void WS2812B::updateLength(uint16_t n)
{
  ...
  // 9 encoded bytes per pixel.
  // 1 byte empty peamble to fix issue with SPI MOSI and 1 byte at the end to clear down MOSI 
  numBytes = (n<<3) + n + 2; // (n<<3) +n is a fast way of doing n*9
  if ((doubleBuffer = (uint8_t *)malloc(numBytes*2))) {
    ...
    *(pixels + numBytes - 1) = 0; // clear the cleardown byte.
...
{
  while (!canShow()); // wait till can show again
  // Start the DMA transfer of the current pixel buffer to the LEDs and return immediately
  spi->dmaSend(pixels, numBytes);

  // Need to copy the last / current buffer to the other half of the double buffer
  // as most API code does not rebuild the entire contents from scratch.
  // Often just a few pixels are changed e.g in a chaser effect
  if (pixels==doubleBuffer) { // pixels was using the first buffer
  ...
  } else { // pixels was using the second buffer
  ...
  while(spi->dmaTransferState() != SPI_STATE_READY); // wait till DMA transfer is finished
  endTime = micros();
}
...
// This does NOT immediately affect what's currently displayed on the LEDs.
// The next call to show() will refresh the LEDs at this level.
// However, this process is potentially "lossy," especially when increasing brightness.
// The tight timing in the WS2811/WS2812 code means there aren't enough free cycles to
// perform this scaling on the fly as data is issued.
// So we make a pass through the existing color data in RAM and scale it
// (subsequent graphics commands also work at thisbrightness level).
// If there's a significant step up in brightness, the limited number of steps (quantization)
// in the old data will be quite visible in the re-scaled version.
// For a non-destructive change, you'll need to re-render the full strip data. C'est la vie.
//------------------------------------------------------------------------------
void WS2812B::setBrightness(uint8_t b)
{
  // Stored brightness value is different than what's passed.
  // This simplifies the actual scaling math later, allowing a fast 8x8-bit multiply and taking the MSB.
  // 'brightness' is a uint8_t, adding 1 here may (intentionally) roll over...so 0 = max brightness,
...
void WS2812B::clear() 
{
	uint8_t * bptr = pixels + 1; // first byte in the buffer is a preable and is always zero. hence the +1
